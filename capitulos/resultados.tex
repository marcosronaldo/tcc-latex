...
Na primeira seção deste capítulo serão discutidos os resultados da análise de distribuição dos dados coletados, definindo para a API Android valores de referencia em cada métrica. Na seção seguinte, apresentamos uma discussão sobre a aplicabilidade de um modelo de regressão para utilizar os dados como um auxílio para desenvolvimento de projetos futuros, principalmente em aplicativos. A terceira seção apresenta uma proposta de validação dos valores de referência definidos na primeira seção, na forma de um cálculo de aproximação de aplicativos ao sistema através de métricas estáticas de código, utilizando distancia euclidiana com média ponderada.
...
%TODO fazer grafico de distribuição para algumas métricas. 

Os gráficos na Figura XXXX demonstram as distribuições dos dados para as métricas XXXXXX.
...
...
\subsection{Resumo da analise de distribuição}
...
\section{Validação dos intervalos de referência com distancia euclidiana}
