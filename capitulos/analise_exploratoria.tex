...
\subsection{Média de linhas de código por método - \textit{Average Method Lines Of Code} (AMLOC)}
...
\subsection{Média de complexidade ciclomática por método - \textit{Average Cyclomatic Complexity per Method} (ACCM)}
...
\subsection{Falta de coesão em métodos - \textit{Lack of Cohesion in Methods} (LCOM4)}
...
\subsection{Conexões aferentes de uma classe - \textit{Afferent Connections per Class} (ACC)}
...
\subsection{Fator de acoplamento - \textit{Coupling Factor} (COF)}
...
Os intervalos aqui não são considerados em função de seus percentis, mas foram as seguintes observações:
